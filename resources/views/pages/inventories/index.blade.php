        ...
        <div id="kt_app_content" class="app-content  flex-column-fluid ">
            ...
            <div id="kt_app_content_container" class="">
                ...
                <div class="card">
                    ...
                    <div class="collapse" id="collapseExample-{{ $inventory->id }}">
                        <div class="card card-body">
                          <table>
                            <thead>
                                ...
                                <th>Terminal</th>
                                <th>Terminal Qty.</th>
                                <th>Terminal Shortage</th>
                            </thead>
                            <tbody>
                                @foreach ($inventory->inventory->stocks as $stock)
                                    <tr>
                                        <td>{{ $stock->product->name }}</td>
                                        <td>{{ $stock->terminal->name }}</td>
                                        <td>{{ $stock->terminal_quantity }}</td>
                                        <td>{{ $stock->terminal_shortage }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                          </table>
                        ...
                        <table class="table align-middle table-row-bordered table-hover fs-6 gy-5" id="kt_datatable_example">
                            <thead>
                                <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                    ...
                                    <th>Seller</th>
                                    <th>Vessel</th>
                                    <th class="text-center">Arrival<br>Date</th>
                                    <th>Product</th>
                                    <th class="text-center">Vessel<br>Qty</th>
                                    <th class="text-center">Landed<br>Qty</th>
                                    <th class="text-end">Shortage</th>
                                    <th class="text-center">Trans.<br>Type</th>
                                    <th>Type</th>
                            ...
                            <tbody class="fw-semibold text-gray-600">
                                @foreach ($inventories as $inv_id => $terminal_stocks)
                                    @php
                                        $bl = \App\Models\InventoryBL::where('inventory_id', $inv_id)->get();
                                        $contracts = \App\Models\LocalContract::where('inventory_id', $inv_id)->get();
                                        $shortage_contract = $bl->sum('bl_quantity') - $bl->sum('landed_quantity');
                                    @endphp
                                        @foreach ($terminal_stocks as $stock)
                                    <tr>
                                        <td hidden>
                                            {{-- <a class="btn btn-primary" data-bs-toggle="collapse" href="#collapseExample-{{ $inventory->id }}" role="button" aria-expanded="false" aria-controls="collapseExample-{{ $inventory->id }}">
                                            </a> --}}
                                        ...
                                        <td class="text-center">
                                            @isset($stock[0]->inventory)
                                                {{ date('M Y', strtotime($stock[0]->inventory->arrival_date)) }}
                                            @endisset
                                        </td>
                                        <td>
                                                {{ $stock[0]->product->code ?? 'N/A' }}
                                        </td>
                                        <td class="text-end">
                                            {{ number_format($stock->sum('terminal_quantity'),3) }}
                                        </td>
                                        <td class="text-end">{{ number_format($stock->sum('terminal_quantity') - $stock->sum('terminal_shortage'), 3) }}</td>
                                        <td class="text-end">
                                                {{ number_format($stock->sum('terminal_shortage'), 3) }}
                                        </td>
                                        <td class="text-center">
                                            @isset($stock[0]->inventory)
                                                {{ $stock[0]->inventory->transaction_type }}
                                            @endisset
                                        </td>
                                        <td>
                                            @isset($stock[0]->inventory)
                                                {{ $stock[0]->inventory->type }}
                                            @endisset
                                        </td>
                                        <td hidden>
                                            @isset($stock[0]->inventory)
                                                {{ $stock[0]->inventory->arrival_date }}
                                            @endisset
                                        </td>
                                        <td class="text-end">
                                            <a href="#"
                                            ...
                                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                                ...
                                                <div class="menu-item px-3">
                                                    @isset($stock[0]->inventory)
                                                    <a href="/inventories/{{ $stock[0]->inventory->id }}/show"
                                                ...
                                                <div class="menu-item px-3">
                                                    @isset($stock[0]->inventory)
                                                    <a href="/inventories/{{ $stock[0]->inventory->id }}/view"
                                                        class="menu-link px-3 text-dark">
                                                        View
                                                    </a>
                                                    @endisset
                                                ...
                                                @else
                                                    ...
                                                    <div class="menu-item px-3">
                                                        @isset($stock[0]->inventory)
                                                        <a href="/inventories/{{ $stock[0]->inventory->id }}/edit"
                                                            class="menu-link px-3 text-dark">
                                                            Edit
                                                        </a>
                                                        @endisset
                                                    ...
                                                    {{-- {!! Form::open([
                                                        'method' => 'DELETE',
                                                        'url' => ['inventory/delete-stock/'.$stock[0]->id],
                                                        'id' => 'delete-form' . '-' . $stock[0]->id,
                ...
                <div class="modal-body">
                        ...
                        <div class="row justify-content-center align-items-center">
                            <table class="table" id="screen-view">
                                ...
                                <tbody>
                                    @foreach ($inventories as $key => $terminal_stocks)
                                        @php
                                            $bl = \App\Models\InventoryBL::where('inventory_id', $key)->get();
                                            $contracts = \App\Models\LocalContract::where('inventory_id', $key)->get();
                                            $shortage_contract = $bl->sum('bl_quantity') - $bl->sum('landed_quantity');
                                        @endphp
                                            @foreach ($terminal_stocks as $stock)
                                        <tr>
                                            ...
                                            <td>{{ $stock[0]->product->code ?? 'N/A' }}</td>
                                            <td>
                                                @isset($stock[0]->inventory)
                                                {{ number_format($stock[0]->inventory->vessel_qty,3) }}
                                                @endisset
                                            </td>
                                            <td>{{ number_format($bl->sum('bl_quantity') - $shortage_contract, 3) }}</td>
                                            <td>{{ number_format($stock->sum('terminal_shortage'), 3) }}</td>
                                            <td>
                                                @isset($stock[0]->inventory)
                                                {{ $stock[0]->inventory->transaction_type }}
                                                @endisset
...
<script>
    ...
    <script>
        ...
        //     var collapse_ele = $(this).find("a:first").attr('href');
        //     $(collapse_ele).collapse("toggle");
        // });
    </script>
    @endpush
</x-default-layout>