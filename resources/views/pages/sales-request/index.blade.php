...
<div class="row">
    ...
    <div class="col-md-6">
        <div class="text-end mt-5">
            <button id="add-new" type="button" class="btn btn-primary btn-sm">Add New</button>
        ...
        <div class="card card-flush mt-5 h-md-100">
            ...
            <form class="pt-1" method="POST" action="{{ route('sales.request.store') }}" id="bls-form" enctype="multipart/form-data">
                @csrf
                @method('POST')
                            ...
                            <div class="rounded-3 p-3">
                                <div class="form-group row gy-3">
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <select class="form-select form-select-sm mb-1" name="inventory_id" id="create_inventory_id" required>
                                            <option>-- Select Vessel --</option>
                                            @foreach($vessels as $vessel)
                                                <option value="{{ $vessel->id }}" data-unlifted-qty="{{ $vessel->unlifted_qty() }}" data-product-id="{{ $vessel->product_id }}">{{ $vessel->vessel->name }} ({{ $vessel->vessel->voyage_number ?? 'LOCAL' }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-12">
                                        <label for="" class="form-label">Vehicle Number</label>
                                        <input type="text" name="vehicle_number" class="form-control form-control-sm" required placeholder="Enter Vehicle Number">
                                    </div>
                                    <input type="hidden" id="contract_qty" name="contract_qty">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                        ...
                                        <div id="kt_docs_repeater_basic">
                                            <!--begin::Form group-->
                                            <div class="form-group">
                                                <div class="" data-repeater-list="terminals">
                                                    <div class="mb-1" data-repeater-item>
                                                        <div class="form-group row gy-3 gx-1 align-items-center">
                                                            <div class="col-md-5">
                                                                <select name="terminal_id" class="form-select form-select-sm create-terminal"> 
                                                            ...
                                                            <div class="col-md-6">
                                                                <input type="number" name="quantity" class="form-control form-control-sm mb-2 mb-md-0 create-quantity" placeholder="Qty" required />
                                                            </div>
                                            ...
                                            <div class="form-group mt-5">
                                                <a href="javascript:;" data-repeater-create class="btn btn-light-primary add-terminal btn-sm fs-7">
                ...
                <div class="text-center">
                    <button type="button" id="check-submit-btn" class="btn btn-lg btn-light fw-bold btn-primary me-2">
                        Submit
                        <div class="spinner-border text-light spinner-border-sm submit-loader" style="display: none" role="status">
                        ...
                        <div class="mb-5">
                            <div class="rounded-3 p-3">
                                <div class="form-group row gy-3">
                                    ...
                                    <div class="col-md-12">
                                        <label class="form-label">Buyer</label>
                                        <select class="form-select form-select-sm mb-1" name="buyer_id" id="buyer_id" required>
                                            <option>-- Select Buyer --</option>
                                            @foreach($buyers as $buyer)
                                                <option value="{{ $buyer->id }}">{{ $buyer->name }}</option>
                                            @endforeach
                                    ...
                                    <div class="col-md-12">
                                        <label for="vessel_id" class="form-label">Vessels</label>
                                        <select class="form-select form-select-sm mb-1" name="inventory_id" id="inventory_id" required>
                                            <option>-- Select Vessel --</option>
                                            @foreach($vessels as $vessel)
                                    ...
                                    <div class="col-md-12">
                                        <label for="vessel_id" class="form-label">Terminals</label>
                                        <select class="form-select form-select-sm mb-1" name="terminal_id" id="terminal_id" required>
                                    ...
                                    <div class="col-md-12">
					                    ...
					                    <input type="number" class="form-control form-control-sm mb-1" placeholder="Quantity" name="quantity" id="quantity" required />
                                    </div>
                                    <div class="col-md-12">
                                        <label for="" class="form-label">Vehicle Number</label>
                                        <input type="text" name="vehicle_number" class="form-control form-control-sm" id="vehicle_number" required placeholder="Enter Vehicle Number">
                                    </div>
  ...
  {{-- Buyer Dashboard --}}
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            <div class="modal-header">
            <h1 class="modal-title fs-5" id="exampleModalLabel">Quantity Status</h1>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <table class="table table-bordered border border-dark table-hover">
                    <thead>
                        <tr>
                            <th>Unlifted Qty.</th>
                            <th>R. Qty</th>
                    ...
                    <tbody>
                        <tr>
                            <td><span id="unlifted-qty"></span></td>
                            <td><span id="requested-qty"></span></td>
                            <td><span id="balance-qty"></span></td>
                        </tr>
                    </tbody>
                </table>
            </div>
  ...
  <script>
                ...
                $("#bls-form").ajaxSubmit(
                    {
                        ...
                        success: function(response) {
                            ...
                            $('#create_inventory_id').closest("span").remove();
                        },
    ...
    <script>
        // $(document).on("change","#create_inventory_id", function () {
            
            // var id = $(this).val();
            // $.ajax({
            //     type: "GET",
            //     url: "{{ url('get-terminal') }}/"+id,
            //     data: {},
            //     success: function (response) {
            //         $('.create-terminal').html(response);
            //     }
            // });
    ...
    <script>
        $(document).on("change","#create_product_id", function () {
            ...
            $.ajax({
                type: "GET",
                url: "{{ url('sales-request/get-inv') }}"+'/'+product_id,
                data: {},
                success: function (response) {
                    $('#create_inventory_id').html(response.vessels); 
                    $('.create-terminal').html(response.terminals);                   
                }
            });
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.message) {
                            ...
                            $('#contract-unlifted-qty').remove();
                        }
                        else {
                            $('#contract_qty').val(response.quantity);
                            $t.before("<span id='contract-unlifted-qty' class='float-end'>Unlifted Qty: "+response.quantity+"</span>");
                        }
                    }
                });
            }
    ...
    <script>
        $(document).on("keyup",".create-quantity", function () {
            ...
            $('#requested-qty').text(value);
            var unlifted_qty = parseFloat($('#unlifted-qty').text());
            var balance_qty = unlifted_qty - value;
            $('#balance-qty').text(balance_qty);
    ...
    <script>
        $(document).on("change","#create_inventory_id", function () {
            ...
            $(this).before("<a href='javascript:void(0);' data-bs-toggle='modal' data-bs-target='#exampleModal'><span class='float-end'>Unlifted Qty: "+unlifted_qty+"</span></a>");
            $('#unlifted-qty').text(unlifted_qty);
        });
    </script>
  @endpush
</x-default-layout>