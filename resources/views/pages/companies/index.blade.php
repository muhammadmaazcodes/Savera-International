	...
	<div class="d-flex flex-column flex-column-fluid">
		...
		<div id="kt_app_content" class="app-content  flex-column-fluid ">
				...
				<div id="kt_app_content_container" class="app-container  container-xxl ">
										...
										<div class="card-toolbar">
											<!--begin::Export dropdown-->
												...
												<div class="d-flex justify-content-end"
														...
														<!--begin::Add customer-->
														<button type="button" class="btn btn-primary ms-2" data-bs-toggle="modal" data-bs-target="#AddCompany">
																Add New Buyer/Sellers
														</button>
														<!--end::Add customer-->
												</div>
												<!--end::Toolbar-->
														...
														<tr>
																...
																<td>
																	<div class="form-check form-switch mb-3">
																		<input class="form-check-input" type="checkbox" disabled {{ ($company->local) == 1  ? 'checked' : ''}}>
																		<label class="form-check-label text-dark"></label>
																	</div>
																	{{-- <span class="fw-bold text-dark">{!! $business->local == 1 ? '<i class="fa fa-check text-dark fs-2"></i>' : '<i class="fa fa-xmark text-dark fs-2"></i>' !!}</span> --}}
																	<span class="d-none">{!! $company->local == 1 ? 'Yes' : 'No' !!}</span>
																</td>
																<td>
																	<div class="form-check form-switch mb-3">
																		<input class="form-check-input" type="checkbox" disabled {{ ($company->international) == 1  ? 'checked' : ''}}>
																		<label class="form-check-label text-dark"></label>
																	</div>
																	{{-- <span class="fw-bold text-dark">{!! $business->international == 1 ? '<i class="fa fa-check text-dark fs-2"></i>' : '<i class="fa fa-xmark text-dark fs-2"></i>' !!}</span> --}}
																	<span class="d-none">{!! $company->international == 1 ? 'Yes' : 'No' !!}</span>
																</td>
																<td>
																	<div class="form-check form-switch mb-3">
																		<input class="form-check-input" type="checkbox" disabled {{ ($company->buyer) == 1  ? 'checked' : ''}}>
																		<label class="form-check-label text-dark"></label>
																	</div>
																	<span class="d-none">{!! $company->buyer == 1 ? 'Yes' : 'No' !!}</span>
																</td>
																<td>
																	<div class="form-check form-switch mb-3">
																		<input class="form-check-input" type="checkbox" disabled {{ ($company->seller) == 1  ? 'checked' : ''}}>
																		<label class="form-check-label text-dark"></label>
																	</div>
																	<span class="d-none">{!! $company->seller == 1 ? 'Yes' : 'No' !!}</span>
																</td>
																<td class="text-center action-td">
																		...
																		@if ($inventories->count() > 0 || $local_contracts->count() > 0)
																			<!--begin::Menu item-->
																					<div class="menu-item px-3">
																						<a href="javascript:void(0);" class="menu-link px-3 delete-has-inv"
																		...
																		@else
																				<div class="menu-item px-3">
																					{!! Form::open(['method' => 'DELETE','route' => ['companies.destroy', $company->id],'id' => 'form-delete'.'-'.$company->id ,'style'=>'display:inline']) !!}
																										{{-- {!! Form::submit('Delete', ['class' => 'menu-link px-3']) !!} --}}
																					{!! Form::close() !!}
																						<a href="javascript:void(0);" onclick="event.preventDefault(); document.getElementById('form-delete-{{ $company->id }}').submit();" class="menu-link px-3"
																								data-kt-customer-table-filter="delete_row">
																								Delete
																						</a>
																				</div>
						...
						<div class="modal fade modal-lg" id="AddCompany" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
							<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
								<div class="modal-content">
									<div class="modal-header bg-primary pt-5 pb-5">
										<h5 class="modal-title text-white fs-2" id="exampleModalLabel">Add New Buyer/Sellers</h5>
										<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
									</div>
									<div class="modal-body">
						
										<form class="pt-1" id="add-company-form" method="POST" action="/companies">
											@csrf
											<div class="row justify-content-center align-items-center">
												<div class="col-md-6">
													<label class="form-label" for="">Company Name<span class="text-danger">*</span></label>
													<input type="text" class="form-control form-control-lg border-dark" placeholder="" name="name" value="{{ old('name') }}" required />
												</div>
												<div class="col-md-3">
													<label class="form-label" for="">Company Code<span class="text-danger">*</span></label>
												...
												<div class="col-md-6 mt-4">
													<div class="form-check form-switch mb-3">
														<input class="form-check-input" name="buyer" type="checkbox" value="1">
														<label class="form-check-label text-dark">Buyer</label>
													</div>
													<div class="form-check form-switch">
														<input class="form-check-input" name="seller" value="1" type="checkbox">
									...
									<div class="modal-footer">
										...
										<button type="button" onclick="$('#add-company-form').submit();" id="submit-add-btn" class="btn btn-primary">Submit</button>
									</div>
