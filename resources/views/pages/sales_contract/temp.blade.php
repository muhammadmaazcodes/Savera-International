...
<div class="card card-flush h-md-100">
	...
	<div class="card-header pt-7">
		...
		<h3 class="card-title align-items-start flex-column">
			<span class="card-label fw-bold text-gray-800">Temporary Contracts</span>
		</h3>
		<!--end::Title-->
		<!--begin::Toolbar-->
		<div class="card-toolbar">
			<a href="/local-contract/temp/create" class="btn btn-sm btn-secondary">Add New</a>
		</div>
		<!--end::Toolbar-->
	</div>
	<!--end::Header-->
			    ...
			    <thead>
			      ...
			      @if ($inventory)
					...
					@if ($inventory)
			            ...
			            <td><a href="{{ route('temp.return_contract_edit',$inventory->id) }}" class="btn btn-sm btn-primary align-self-center">Return</a></td>
						@endif
					@else
					<td><a href="{{ route('temp.return_contract',$contract->id) }}" class="btn btn-sm btn-primary align-self-center">Return</a></td>
					@endif
					...
					<td>
						...
						{!! Form::open(['method' => 'DELETE','route' => ['local-contract.destroy', $contract->id],'style'=>'display:inline']) !!}
							...
							{!! Form::close() !!}
						<a href="{{ url('/temporary-contract/edit/'.$contract->id) }}" class="btn btn-primary btn-sm">Edit</a>
						@endif
					</td>
