<x-default-layout>
  ...
  <div class="row">
    ...
    <div class="col-md-6">
      ...
      <div class="card">
        <div class="card-body">
            <form action="{{ url('custom-payments/update/'.$custom_payment->id) }}" id="edit-form" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row mb-5">
                    <div class="col-md-6 mb-2">
                      ...
                      <select name="buyer_id" id="buyer_id" class="form-select form-select-sm border border-dark" required>
                        ...
                        @foreach ($buyers as $buyer)
                            <option value="{{ $buyer->id }}" {{ $custom_payment->buyer_id == $buyer->id ? 'selected' : '' }}>{{ $buyer->name }}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="col-md-6 mb-2">
                      ...
                      <select name="inventory_id" id="inventory_id" class="form-select form-select-sm border border-dark">
                        ...
                        @foreach ($inventories as $inventory)
                          <option value="{{ $inventory->id }}" {{ $custom_payment->inventory_id == $inventory->id ? 'selected' : '' }}>{{ $inventory->vessel->name }} ({{ $inventory->voyage_number }})</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                      ...
                      <select name="bank_acc_title" id="bank_acc_title" class="form-select form-select-sm border border-dark">
                        ...
                        @foreach ($bank_accounts as $bank)
                          <option value="{{ $bank->account_title }}" {{ $custom_payment->bank_acc_title == $bank->account_title ? 'selected' : '' }}>{{ $bank->account_title }}</option>
                        @endforeach
                      </select>
                    </div>
                </div>
                ...
                <div class="row mb-5 justify-content-center">
                  <div class="col-md-4 mb-2">
                    ...
                    <select name="bank_id" class="form-select form-select-sm border border-dark" id="bank_id" required>
                      <option value="">---</option>
                      @foreach ($bank_accounts as $bank)
                        <option value="{{ $bank->id }}" {{ $custom_payment->bank_id == $bank->id ? 'selected' : '' }} >{{ $bank->bank_name }}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="col-md-4">
                    <label for="bank_branch" class="form-label">Bank Branch</label>
                    <input type="text" name="bank_branch" id="bank_branch" value="{{ $custom_payment->bank_branch }}" class="form-control form-control-sm border border-dark">
                  </div>
                  <div class="col-md-4">
                    <label for="deposit_slip_number" class="form-label">Deposit Slip #</label>
                    <input type="text" name="deposit_slip_number" id="deposit_slip_number" value="{{ $custom_payment->deposit_slip_number }}" class="form-control form-control-sm border border-dark">
                  </div>
                  <div class="col-md-6">
                    <label for="stamp_date" class="form-label">Stamp Date</label>
                    <input type="date" name="stamp_date" id="stamp_date" value="{{ $custom_payment->stamp_date }}" class="form-control form-control-sm border border-dark">
                  </div>
                  <div class="col-md-6">
              ...
              <div id="kt_docs_repeater_basic">
                  <div class="form-group">
                      ...
                      <div class="row border border-dark p-3 rounded-3 mb-2">
                        <input type="hidden" id="cheque-form" value="{{ route('update.cheque',$custom_payment->cheques->id) }}">
                        <div class="col-md-4">
                          <label for="cheque_number" class="form-label">Cheque #</label>
                          <input type="text" name="cheque_number" value="{{ $custom_payment->cheques->cheque_number }}" id="cheque_number" class="form-control form-control-sm">
                        </div>
                        <div class="col-md-4">
                          <label for="amount" class="form-label">Amount</label>
                          <input type="number" name="amount" id="amount" value="{{ $custom_payment->cheques->amount }}" class="form-control form-control-sm">
                        </div>
                        <div class="col-md-4">
                          <label for="clearing_date" class="form-label">Clearing Date</label>
                          <input type="date" name="clearing_date" value="{{ $custom_payment->cheques->clearing_date }}" id="clearing_date" class="form-control form-control-sm">
                        </div>
                        <div class="col-md-6">
                          <label for="remarks" class="form-label">Remarks</label>
                          <input type="text" name="remarks" id="remarks" value="{{ $custom_payment->cheques->remarks }}" class="form-control form-control-sm">
                        </div>
                        <div class="col-md-6">
                          <p></p>
                          <button type="button" id="submit-cheque-form" class="btn btn-light-dark border border-dark btn-sm mt-4">Update</button>
                        </div>
                    </div>
...
<script>
    $(document).on("click","#submit-cheque-form", function () {
      
        var url = $('#cheque-form').val();
        alert(url)
